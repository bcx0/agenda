...
/**
 * Soustrait une plage [cut.startTime, cut.endTime) d'une liste de plages :
 * - plage entièrement couverte → supprimée ;
 * - coupe au milieu → scindée en deux ;
 * - coupe sur un bord → tronquée.
 * Utilisé par le bouton "Retirer ce créneau" du panneau jour admin.
 */
export function subtractRange(ranges, cut) {
  const cutStart = parseTimeToMinutes(cut?.startTime);
  const cutEnd = parseTimeToMinutes(cut?.endTime);
  ...
  for (const range of mergeRanges(ranges)) {
    ...
    // Pas de chevauchement → conservée telle quelle
    if (cutEnd <= start || cutStart >= end) {
      result.push([start, end]);
      continue;
    }
    // Morceau avant la coupe
    if (start < cutStart) result.push([start, cutStart]);
    // Morceau après la coupe
